  ...
  return createPortal(
    <div className="fixed inset-0 bg-surface-overlay backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
      <div className="bg-surface-card border border-border-default rounded-xl p-6 w-full max-w-md shadow-2xl relative">
        ...
        <h3 className="text-xl font-bold text-heading mb-6">{isEditing ? 'Edit Campaign' : 'New Campaign'}</h3>
        <form onSubmit={handleCreateCampaign} className="space-y-4">
          ...
          <div className="pt-4 flex justify-end">
            ...
            <button type="submit" disabled={!newCampaign.name?.trim()} className="px-5 py-2 bg-accent hover:bg-accent-hover text-heading font-medium rounded transition-colors disabled:opacity-50">{isEditing ? 'Save' : 'Create'}</button>
          </div>
        </form>
      </div>
    </div>,
